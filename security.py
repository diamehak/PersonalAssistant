...
from modules.user_handler import UserData  # Adjust the path as necessary

from modules.user_handler import face_unlocker as FU
...
def startLogin():
    try:
        result = FU()  # Call the face unlocker function
        if result:  # If face is recognized
            user = UserData()
            user.extractData()
            userName = user.getName().split()[0]
            welcLbl['text'] = f'Hi {userName},\nWelcome to the world of\nScience & Technology'
            ...
            loginStatus['text'] = 'LOCKED'
            faceStatus['text'] = '(Face Not Detected)'

...
def record(clearChat=True, iconDisplay=True):
    ...
    # Function to safely update the label on the main thread
    def update_label(text):
        AITaskStatusLbl['text'] = text

    # Schedule the initial label update
    AITaskStatusLbl.after(0, update_label, 'Listening...')
    ...
    with sr.Microphone() as source:
        ...
        said = ""
        try:
            # Schedule the processing label update
            AITaskStatusLbl.after(0, update_label, 'Processing...')
...
def showSingleImage(type, data=None):
	...
	try:
		...
		pass
	img1 = ImageTk.PhotoImage(Image.open('assets/images/heads.jpg').resize((220,200), Image.LANCZOS))
	img2 = ImageTk.PhotoImage(Image.open('assets/images/tails.jpg').resize((220,200), Image.LANCZOS))
	...
	if type=="weather":
		...
		img3 = ImageTk.PhotoImage(Image.open('assets/images/dice/'+type+'.jpg').resize((200,200), Image.LANCZOS))
